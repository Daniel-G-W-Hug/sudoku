...
list<int> sudoku_concatenate_candidate_lists_in_subregion(const Sudoku& s,
  ...
  for (int j=0;j< s.region_size; ++j) {  // for each cell in subregion
    ...
    if (s(cnt).val == 0) { // if cell is empty, there should be candidates
      concatenated_list.insert(concatenated_list.end(),
			       s(cnt).cand.begin(),s(cnt).cand.end());
    }
  }   
...
vector<int> sudoku_count_candidate_entries(const Sudoku& s, const std::list<int>& input_list) {
...
vector<list<int>> sudoku_candidate_positions_in_subregion(
                  const Sudoku& s,
                  const Region region,
                  const int subregion,
...
tuple< int, vector<int>, vector<int> >
  ...
  return make_tuple(num_hidden_singles_in_subregion,hidden_single_values,hidden_single_pos);
}


// identify naked twins in subregion (index i)
tuple< int, vector<list<int>>, vector<list<int>> >
      identify_naked_twins_in_subregion(const Sudoku& s,
                                        const Region region,
                                        const int subregion) {
  ...
  for (int j=0;j < s.region_size; ++j) {     
	...
	if (s(cnt_k).val == 0 && s(cnt_k).cand.size() == 2 &&
	    s(cnt).cand == s(cnt_k).cand &&
            (cand_count[s(cnt).cand.front()-1] > 2 || cand_count[s(cnt).cand.back()-1] > 2) ) {
	  // naked twin found
...
// identify hidden twins in subregion (index i)
tuple< int, vector<list<int>>, vector<list<int>> >
      identify_hidden_twins_in_subregion(const Sudoku& s,
                                         const Region region,
                                         const int subregion) {
