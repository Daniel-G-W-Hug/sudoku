...
// update required entry lists of all affected cells
...
int  sudoku_num_entries(const Sudoku& s);  // return no. of entries != 0 (non empty entries)
...
// just reduces candidate lists of cells in region, does not fill in cells
int sudoku_remove_hidden_twins_in_region(Sudoku& s, int region);
